...
{ int n,i;
  Empty();
  n = nCol/(GetRandMode()?32:16); //число обращений к генератору баз.вектора -1
  for (i=0; i<n; i++) Concat(GetRandV());
  i = nCol - n*(GetRandMode()?32:16);
  if (i!=0) Concat((const BYTE*)GetRandV(), i);
  return *this;
}
...
CBV CBV::GenRbvMid(int nCol, int nRang)
{
  ...
  CBV  bv_Con (1, nCol, 1); 
  int d=0, e, i, j, log = 4; 
  CBV  Rbv(0,nCol,1), bv = Rbv, bvCon = Rbv;
